...
class _GameBoardState extends State<GameBoard> {
  ...
  void gameLoop(Duration frameRate) {
    Timer.periodic(frameRate, (timer) {
      setState(() {
        ...
        currentPiece.movePiece(values.Direction.dowm);
      });
    });
  }
  ...
  bool checkCollision({values.Direction? direction}) {
    return currentPiece.position.any((pos) {
      ...
      if (direction == values.Direction.dowm) row += 1;
      return row >= columnLength ||
          col < 0 ||
          col >= rowLength ||
...
void checkLanding() {
  if (checkCollision(direction: values.Direction.dowm)) {
  ...
  void resetGame() {
    gameBoard = List.generate(
        columnLength, (_) => List.generate(rowLength, (_) => null));
    currentScore = 0;
    gameOver = false;
    createNewPiece();
    startGame();
